...
const CreateGroupModal = ({ children, isEditing }) => {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      const { data } = await axios.get(`https://chatter-backend-nine.vercel.app/api/user/allUsers?search=${search}`, {
        headers: {
    ...
    if (groupName && selectedUsers.length >= 2) {
      try {
        const { data } = await axios.post(
          "https://chatter-backend-nine.vercel.app/api/chat/createGroup",
          {
            name: groupName,
  ...
  const handleAdd = async () => {
    ...
    try {
      const { data } = await axios.put(
        "https://chatter-backend-nine.vercel.app/api/chat/addToGroup",
        {
